...
type VariableList = {[key: string]: any};
...
const parseComments = (contents: String) => {
    ...
    for (const line of lines)  {
        for (const ch of line.trim()) {
            if (ch === commentToken && i === 0) { // Whole line comment
                state = STATES.COMMENT;
                captureNext = true;
                i++;
                continue;
            }
            if (ch !== commentToken && i === 0) { // Variable assignment
                state = STATES.VARIABLE;                
            }
            if (ch === commentToken && i > 0) { // Inline comment
                state = STATES.COMMENT;
                captureNext = false;
                lastComment = "";
                i++;
            ...
            if (ch === assignmentToken && state === STATES.VARIABLE && captureNext && lastComment.length > 0) { // End of assignment with a comment on the previous line
            ...
            if (ch === assignmentToken && state === STATES.VARIABLE && !captureNext && i > 0) { // Variable assignment completed
                state = STATES.NONE;
            }
